...
ggplot() +
  geom_polygon(data = WorldMap, aes(x=long, y = lat, group = group), fill=NA, color = "white", alpha=0.9) +
  ...
  scale_size_continuous(name="Number of Specimens", trans="log", range=c(3,10), breaks=mybreaks) +
    ...
    scale_color_viridis(trans="log", breaks=mybreaks,name="Number of Specimens" ) +
  theme_void() +
...
sp3 <- ggplot(NanoDrop_data, aes(x=nanodrop_concentration, y=concentration)) +
  geom_point() +
  geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) 
sp3 + stat_cor(method = "pearson", label.x = 100, label.y = )
```
