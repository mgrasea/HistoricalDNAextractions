...
library(hrbrthemes)
sp1 <- ggplot(HDNA_data, aes(x=year_collected, y=concentration_norm)) +
  geom_point() +
  geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) +
  theme_ipsum()

sp1 + stat_cor(method = "pearson", label.x = 50, label.y = 150)
```
...
PowerSoil <- filter(HDNA_data, contemporary == "no", dna_extraction == "Qiagen DNeasy PowerSoil Pro Kit" )
```
...
view(hDNA_extract)
ggplot(hDNA_extract, aes(x=as.factor(dna_extraction), y=concentration_norm)) + 
    geom_boxplot(fill="slateblue", alpha=0.2) + 
    xlab("DNA extraction")
```

```{r box plot purity}
ggplot(NanoDrop_data, aes(x=as.factor(contemporary), y="260/230")) + 
    geom_boxplot(fill="slateblue", alpha=0.2) + 
    xlab("Contemporary")
```
...
sp3 + stat_cor(method = "pearson", label.x = 100, label.y = 100)
```
